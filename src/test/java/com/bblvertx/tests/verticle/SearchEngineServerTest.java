...
public class SearchEngineServerTest extends AbstractTest {
	...
	/**
	 * Pré-conditions : initialisation du verticle avec injection de dépendance
	 * via HK2 / CDI.
	 * 
	 * @param context
	 */
	...
	public void before(final TestContext context) {
		vertx = Vertx.vertx();
		DeploymentOptions opt = new DeploymentOptions().setConfig(new JsonObject(FileUtils.file2stringQuietly(getDataDir() + "config.json")));
		vertx.deployVerticle("java-hk2:" + SearchEngineServer.class.getName(), opt, context.asyncAssertSuccess());
	...
	/**
	 * Test nominal : on vérifie l'adhérence du serveur avec les routes
	 * configurées dans routes.properties.
	 * 
	 * @param context
	 */
	@Test
	public void testHelloWorld(final TestContext context) {
		...
		String requestURI = "/prodit-se/api/" + propReader.getQuietly(ROUTE_CONFIG_FILE, String.format(KEY_TPL_ROUTE_URL, "1"));
	...
	public void testSearchElasticSearch(final TestContext context) {
		...
		Integer port = propReader.getIntQuietly(APP_CONFIG_FILE, KEY_PORT);
		String host = "localhost";
		String requestURI = "/prodit-se/api/" + propReader.getQuietly(ROUTE_CONFIG_FILE, String.format(KEY_TPL_ROUTE_URL, "3")) + "?startIndex=0&maxResults=100";
