...
import com.bblvertx.indexation.adapter.IndexationAdapter;
 ...
 *
 */
public abstract class AbstractIndexationSingleRoute<T extends Serializable> extends AbstractIndexationRoute {
	/**
	 * Adapter pour les spécificités de chaque indexation.
	 */
	protected IndexationAdapter<T> adapter;
	 ...
	 * @param ctx
	 */
	public AbstractIndexationSingleRoute(String url, String contentType, Router router, RouteContext ctx) {
	...
	private void indexationNewData(String id) {
		try {
			...
			do {
				...
				lstResults = ctx.getDataSource().execute(sql, pId.asList(pRsSearch.asList(pLimit.asList(pOffset.asList()))), mapper);

				if (isNotEmpty(lstResults)) {
					for (T elem : lstResults) {
						ctx.getEsClient() //
						        .getClient() //
						        .prepareIndex(adapter.getIndexName(), adapter.getIndexType(), adapter.getId(elem) + "_" + adapter.getLanguage(elem)) //
	 ...
	 * @return the adapter
	 */
	public IndexationAdapter<T> getAdapter() {
		return adapter;
	}
	 ...
	 *            the adapter to set
	 */
	public void setAdapter(IndexationAdapter<T> adapter) {
