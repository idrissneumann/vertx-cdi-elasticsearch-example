...
import com.bblvertx.indexation.adapter.IndexationDeltaAdapter;
 ...
 */
public abstract class AbstractIndexationDeltaRoute<T extends Serializable> extends AbstractIndexationRoute {
	/**
	 * Adapter pour les spécificités de chaque indexation.
	 */
	protected IndexationDeltaAdapter<T> adapter;
	 ...
	 * @param ctx
	 */
	public AbstractIndexationDeltaRoute(String url, String contentType, Router router, RouteContext ctx) {
	...
	private void purgeOldData() {
		try {
			...
			do {
				...
				lstResults = ctx.getDataSource().execute(sql, pRsSearch.asList(pLimit.asList(pOffset.asList())), adapter.getIdMapper());
				...
				if (isNotEmpty(lstResults)) {
					for (Integer idElem : lstResults) {
						for (String lng : CODE_LANGUAGES) {
							ctx.getEsClient() //
							        .getClient() //
							        .prepareDelete(adapter.getIndexName(), adapter.getIndexType(), String.valueOf(idElem + "_" + lng)) //
							        .execute().actionGet();
						}
	...
	private void indexationNewData() {
		try {
			...
			do {
				...
				lstResults = ctx.getDataSource().execute(sql, pRsSearch.asList(pLimit.asList(pOffset.asList())), mapper);

				if (isNotEmpty(lstResults)) {
					for (T object : lstResults) {
						ctx.getEsClient() //
						        .getClient() //
						        .prepareIndex(adapter.getIndexName(), adapter.getIndexType(), adapter.getId(object) + "_" + adapter.getLanguage(object)) //
			...
			if (idElems.length() > 0) {
				...
				ctx.getDataSource().executeUpdate(String.format(sqlUpdate, idElems.toString()), pRsSearch2.asList());
			}
		} catch (Exception e) {
			throw new TechnicalException(e);
		}
	 ...
	 * @return the adapter
	 */
	public IndexationDeltaAdapter<T> getAdapter() {
		return adapter;
	}
	 ...
	 *            the adapter to set
	 */
	public void setAdapter(IndexationDeltaAdapter<T> adapter) {
